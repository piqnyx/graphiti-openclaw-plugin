...
**Capture.** After every agent run the plugin reads OpenClaw's own transcript store — the SQLite database the gateway writes — and takes the rows it has not taken before, keeping only `user`/`assistant` messages and buffering them per dialog. The cursor is a session id, a row number and the set of message ids already captured, so a rewind (which copies the kept history into a fresh session) captures only what is genuinely new. Nothing is read from the hook payload, because a message rewritten between deliveries used to make "what is new" unanswerable; and nothing is lost to a missed turn, because the rows stay on disk and the next read resumes from the cursor. A batch is submitted to Graphiti when it reaches `bufferLimit` messages or the dialog goes quiet for `bufferTimeout` seconds. Each dialog becomes one Graphiti saga; each batch becomes one episode in it, chained to its predecessor. Unsent batches survive a gateway restart in a durable spool.
