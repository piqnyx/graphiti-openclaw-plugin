...
```text
graphiti_search            факты, сущности и эпизоды одним поиском, со скорами и якорями
graphiti_browse            текст диалога вокруг одного или нескольких якорей
graphiti_note              заметка в текущий батч диалога
graphiti_status            здоровье backend, размер графа, проверки целостности
```

`graphiti_search` и `graphiti_browse` — две ступени одного движения. Первая отвечает, что известно, вторая — как это было сказано.

Поиск идёт одним запросом к форк-тулзе `search_memory_combined`: она гоняет рецепт `COMBINED_HYBRID_SEARCH_RRF` и возвращает три списка — факты, сущности, эпизоды — **вместе со скорами реранкера**. Upstream их выбрасывает, оставляя только порядок; без чисел вторая ступень невозможна, потому что агенту нечем выбрать, что разворачивать. Реранкер именно RRF: рецепт по умолчанию переранжирует cross-encoder'ом, а это вызов модели на каждый поиск.

Лимиты по типам применяются на стороне плагина: сервер принимает один `limit`, а запрос «десять фактов и ни одной сущности» не должен стоить второго round-trip. Ноль исключает тип целиком.

Якорь — это имя эпизода, а не uuid: uuid агенту бесполезен, его нельзя передать обратно. Сопоставление uuid → имя делается одним вызовом `get_episodes_by_ref` на всю выдачу. Рядом с якорем стоит число — сколько результатов этой выдачи на него ссылаются; это единственная величина при якоре, которая что-то значит для выбора, и читается она без знания устройства графа.

У сущности собственного провенанса нет, поэтому её якоря выводятся из фактов, которые её касаются: `search_memory_combined` отдаёт `source_node_uuid` и `target_node_uuid` каждого ребра.

Факт с проставленным `invalid_at` по умолчанию скрыт: он не ложь, но уже вытеснен более новым, и повторять его как текущее знание нельзя. Параметр `include_outdated` возвращает такие факты с пометкой.

`graphiti_browse` берёт список якорей и читает вокруг каждого. Соседи адресуются по номеру батча в имени (`<saga tail>-<n>`), поэтому окно вверх и вниз — это просто `n±k`. Пределы — в конфиге (`browseChars`, `browseMaxChars`, `browseMaxEpisodes`, `browseMaxTotalChars`); то, что не поместилось, объявляется опущенным, а не исчезает молча.
